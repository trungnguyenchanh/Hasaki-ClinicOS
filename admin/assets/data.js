/* ===== Hasaki ClinicOS — store dữ liệu khách (demo, đọc từ JSON) =====
   Dữ liệu thật nằm ở: admin/data/customers.json
   data.js chỉ nạp JSON đó và giữ NGUYÊN API cũ (getCustomer / linkFor)
   để 40 trang không phải sửa gì.

   Cách nạp: thử fetch JSON đồng bộ (XHR sync) khi khởi tạo. Nếu chạy qua
   http(s):// → đọc được file JSON. Nếu mở trực tiếp file:// và trình duyệt
   chặn → tự dùng FALLBACK inline bên dưới (vẫn đủ 3 khách demo).
   Nối API thật: trỏ DATA_URL sang endpoint trả JSON theo cùng cấu trúc.
*/
